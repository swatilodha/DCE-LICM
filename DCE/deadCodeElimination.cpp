...
public:
  ...
  virtual bool runOnFunction(Function &F) {
    ...
    DeadCodeEliminationAnalysis *dce = new DeadCodeEliminationAnalysis(domain.size(), BACKWARD, boundaryCond, initCond);
 ...
 private:
  ...
  void eliminateDeadCode(Function &F, map<BasicBlock *, struct bbProps *> result) {
  	  for (Function::iterator itr = F.begin(); itr != F.end(); ++itr) {
  	  	...
  	  	for (BasicBlock::reverse_iterator rItr = itr->rbegin(); rItr != itr->rend(); ++rItr) {
  	  		Instruction* I = &(*rItr);
  ...
  */
  void populateInfoMap(Function &F) {
    ...
    for (po_iterator<BasicBlock*> itr = po_begin(&F.getEntryBlock()); itr != po_end(&F.getEntryBlock()); ++itr) {
        ...
        for (BasicBlock::reverse_iterator rItr = basicBlk->rbegin(); rItr != basicBlk->rend(); ++rItr) {
        	Instruction *I = &(*rItr);
        	...
        	// gen
        	auto foundInDomain = find(domain.begin(), domain.end(), I);
        	if (foundInDomain != domain.end() && !(info->killSet[domainToBitMap[I]])) {
  ...
  void setupDomain(Function &F) {
  	for (auto &BB : F) {
        for (auto &I : BB) {
            Instruction *ins = &I;
            if (!isLive(ins)) 	domain.push_back(ins);
        }
    }
  }
  ...
  public:
    DeadCodeEliminationAnalysis(int domainSize, enum passDirection dir, BitVector boundaryCond,
             BitVector initCond)
...
RegisterPass<DeadCodeElimination> X("dead-code-elimination", "ECE/CS 5544 Dead-Code-Elimination Pass");
} // namespace llvm
